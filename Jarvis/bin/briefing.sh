...
if [ "$MODE" = "morning" ]; then
  ASK='Compose Rob'\''s MORNING briefing. Check: today'\''s Trello "To Do"/"In Progress" cards (`Jarvis/bin/trello.sh cards "To Do"` and `... "In Progress"`), Ops/Tasks.md "Now" items, anything blocked in Ops/Proposals.md that needs his call, and yesterday'\''s daily log for open follow-ups. Keep it tight: 3-6 short lines, chat-shaped for Telegram, no headers/markdown tables. Lead with anything that actually needs him today; skip sections with nothing to say.'
else
  ASK='Compose Rob'\''s EVENING briefing. Check today'\''s daily log (Daily/YYYY-MM-DD.md) for what actually happened, any Trello cards moved/blocked today, and anything left open in Ops/Proposals.md. Keep it tight: 3-6 short lines, chat-shaped for Telegram, no headers/markdown tables. Focus on what moved and what'\''s still waiting on him.'
fi
