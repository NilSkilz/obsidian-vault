...
4. Infra check: run `ssh proxmox "lvs pve/data --noheadings -o data_percent"` and `ssh proxmox "pct list; qm list"`. If the thin pool Data% is over 85, or any LXC/VM that should be running is stopped/io-error, that counts as "actually broken" (see rule 7) — on 2026-07-03 the pool hit 100% and paused the HAOS VM with io-errors. Quick fix for a full pool: `pct fstrim` every running LXC (a weekly host cron does this; it can be run early). Also check `df /data` stays under 90% (note: `/data` is a bind mount of the host's data1 backup disk, so this is really "the backup disk is filling", see Infrastructure.md; when it trips, count dumps in `/mnt/data1/backups/dump` rather than hunting for files in the vault).
5. Updates: run `Jarvis/bin/updates.sh heartbeat`. It self-throttles to once a day (prints a skip message on other runs) and checks + applies updates across the Proxmox host, every LXC, and Home Assistant (add-ons, core, OS). A HAOS VM reboot during an OS update is expected, not an outage. Summarise what it updated in the daily log; treat any NEEDS-ROB line in its output as "actually broken" (rule 7).
6. Keep your eyes open throughout. If anything you saw along the way strikes you as odd, interesting, or worth a human remark, even though no rule above asked about it, note it in the daily log (and in Ops/Proposals.md if it needs Rob). Rob has explicitly asked for noticing over rule-following (2026-08-24).
7. Append a short entry to today'\''s daily log (Daily/YYYY-MM-DD.md, create the file only if this is the first real entry today) noting what you checked and did this run. Keep it to a few lines. If nothing needed doing, a one-line "nothing due" note is enough — do not pad it.
8. Do not message Rob directly from this run. Silence is fine unless something is actually broken or needs him.'
