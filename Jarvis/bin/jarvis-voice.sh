...
#   voice: OpenAI TTS voice, default "fable" (alloy|ash|ballad|coral|echo|fable|onyx|nova|sage|shimmer|verse)
...
TEXT="$1"; VOICE="${2:-fable}"; CHAT="${3:-$TELEGRAM_CHAT_ID}"
...
BODY=$(python3 -c 'import json,sys; print(json.dumps({
  ...
  "voice": sys.argv[2],
  "input": sys.argv[1],
  "instructions": "Warm, dry, British. Conversational and a little wry, like a clever friend, not a customer service agent. Natural pace.",
...
"$HOME/bin/ffmpeg" -y -loglevel error -i "$TMP/say.mp3" -c:a libopus -b:a 32k -ar 48000 -ac 1 "$TMP/say.ogg" || exit 1
