...
1. ~~Measure the v1 rail~~ **DONE 22 Sept, and the answer kills the reuse plan.** Photo shows it's not a linear rail at all: a single round steel rod, roughly 8mm, ~50cm long, with a plastic bushing carriage and what looks like an optical endstop on a flying lead. Classic scanner/printer salvage. A single round rod gives no rotational constraint (the carriage spins on it) and a plastic bushing won't take repeated axial thrust. **Verdict: unusable as the machine axis. Buy a proper rail:** MGN12H 500mm + carriage plus GT2 10mm belt, 20T 8mm-bore pulley (iHSV57 shaft) and idler, ~£35-45 total. On the Wishlist. Silver lining: 50cm was the right length instinct, and the endstop might get reused for homing.
...
- **Reuse the v1 linear rail.** ~~Rob confirmed the rail survived~~ SUPERSEDED 22 Sept: the surviving part is a scanner rod, not a load-bearing rail (see Phase 1). The architecture decision stands (**belt-driven rail**, OSSM-style), but the rail itself is a new purchase: MGN12H 500mm.
