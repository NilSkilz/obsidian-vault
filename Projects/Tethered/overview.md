...
- **SEO/crawler infra shipped 2026-09-14:** `scripts/generate-sitemap.ts` (build-time sitemap.xml + robots.txt, 6deb539) and `scripts/prerender.ts` (build-time static HTML per blog post — title/meta/OG/JSON-LD, react-markdown server-side, fails soft if API unreachable, 4010fc8). Needed the Amplify rewrite to serve real files first, SPA catch-all only as last resort. The 2026-09-15 `404-200` rule did that but as a side effect made every SPA deep link (`/dashboard`, `/journal`, invite links) return HTTP 404 (with the correct app-shell body, so real users/browsers were unaffected, but Cypress `cy.visit()` and crawlers both treated it as broken) — 12 of the 16 e2e failures on 2026-09-17 traced to this. **Current (fixed 2026-09-17 12:10):** Amplify rewrite is a plain **200 rewrite** to `/index.html` gated by an extension-exclusion regex (`</^[^.]+$|\\.(?!(css|js|png|jpg|jpeg|gif|ico|svg|txt|xml|json|map|webmanifest|woff|woff2|ttf|eot)$)([^.]+$)/>`) — any extensionless path (SPA route) gets 200+index.html, anything with a known static extension is served as the real file. Verified: SPA routes 200 app-shell, llms.txt/robots.txt/version.txt 200 text/plain, sitemap.xml 200 text/xml, nonexistent assets still 404.
