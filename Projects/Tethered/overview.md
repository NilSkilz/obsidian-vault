...
- **Signed-in users saw an empty blog (fixed 2026-09-15, d84af13, develop):** public blog pages hardcoded `authMode: "identityPool"`. Logged-out visitors and crawlers get the guest IAM role (fine), but signed-in users get the authenticated identity-pool role, which `allow.guest()` doesn't cover, so AppSync returned zero posts — prerendered HTML painted first, then React refetched and wiped it. `AmplifyAdapter.ts` already has a helper for this exact trap (`getPublicReadClient`, picks userPool when signed in); the blog pages just weren't using it. **Lesson: any "public" page on Tethered must use `getPublicReadClient`, not a hardcoded authMode, or logged-in users see broken data.** Needs merge to main to reach prod.
