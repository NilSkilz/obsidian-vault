...
## Power: 18650 battery shield option (proposed 2026-08-14)

Rob's pivot after the custom-PCB route stalled: drop the mains 12V brick + buck and run the whole rig off a rechargeable **18650 battery shield** that outputs both 5V (USB, for the ESP32) and 12V (for the pumps). Verdict: **good idea, and arguably the better topology.**

- **Safety win, lead with it:** no mains anywhere near saline + needles + a person. A wall brick puts a mains PSU next to a wet bodyplay scene; a battery pack removes that entirely. This is a real upgrade, not just convenience.
- **Current is a non-issue.** Each pump is a 500-series 12V peristaltic vetted at **250–300mA at 12V**, and we run them at ~20% throttle (17 ml/min working). Two pumps ≈ 0.5–0.6A *peak*, far less average. Logic (ESP32 + 2× GC9A01 + 2× HX711) ≈ 0.3–0.5A at 5V. Total draw is only ~4–8W. Any half-decent shield's 12V output covers this with room to spare — just **confirm the board's 12V rating** (some cheap ones cap ~0.5–1A; still enough, but check).
- **Runtime = roughly one run per cell.** A full session is ~1L/side in 60 min = ~60 min of pumping ≈ 5–8Wh. A single 18650 holds ~10–12Wh, so **one cell = one run with slim margin** after boost losses. Want back-to-back sessions or headroom → use a **2- or 3-cell shield**. A 3S pack also sits at ~11–12.6V and can drive the pumps *directly* (no boost, more efficient); a peristaltic at 20% throttle won't notice 11V vs 12V.
- **ESP over USB from the shield's 5V = good clean call (Rob's instinct).** Keeps the logic on the regulated 5V USB rail, isolated from the switched/motor-noisy 12V pump rail — helps the HX711 noise story we already care about. Keep the flyback + a bulk cap on the 12V rail near the pumps so motor spikes don't ride back into the boost.
- **The heater kills this.** If the v2 inline saline heater block goes in, that PTC element is a 10–30W load that dwarfs everything else and would flatten a small pack fast + overtax the boost. Battery is great for **pumps-only v1**; if heating gets added, either a much bigger pack or keep the heater on mains. Flag at v2.
- **E-stop unchanged:** still an NC button in the 12V pump rail, now between the shield's 12V out and the pump MOSFETs. Battery doesn't remove the need for the hardware backstop.
- **Charging while running:** many cheap shields disable the 12V boost while charging over USB. If Rob wants plug-in-mid-run backup, verify on the specific board; otherwise run on battery, charge between sessions.
- **BOM impact:** replaces line #2 (12V brick) + #3 (MP1584 buck) with one shield. Does **not** replace the MOSFET drivers / flyback / e-stop — those stay, just fed from the shield instead of a brick. Net = simpler and rechargeable.
...
Jarvis's generated schematic (`pcb/` — gen_schematic.py + .kicad_sch) **didn't import cleanly and has been deleted at Rob's call.** Rob is drawing the KiCad schematic from scratch himself. The pinout below still stands as the agreed v1 — Rob's board uses these assignments.

**Encoders are KY-040 modules (5-pin: GND / + / SW / DT / CLK)**, wired to the encoder GPIOs below. Mapping: **CLK→A, DT→B, SW→switch**. Power **+ at 3V3, not 5V** — the KY-040's onboard 10k pull-ups sit on CLK/DT to VCC, so at 5V they'd push 5V into ESP32 pins (not 3.3V-tolerant). SW has **no onboard pull-up**, so enable the ESP32 internal pull-up on the SW pin in firmware.
...
**Soldering iron (2026-09-07):** Rob's current iron's fine tip won't get hot enough for the board's finer joints — diagnosed as a heat-transfer problem (separate heater can't feed a skinny tip fast enough), not a temperature setting, so cranking the dial just oxidises the tip. Fix is a cartridge-tip iron (heater inside the tip). Researched and recommended: **Pinecil V2, £53 from Test Clips UK** (official GB reseller) over the FNIRSI HS-02A (£42) — bigger community, open firmware, official supply chain, £11 apart. Runs off Rob's existing 96W USB-C Mac charger (full ~88W profile). Needs a ~2mm **chisel** tip for this board's through-hole joints (not a needle tip, that's for SMD). **UPDATE 2026-09-10: Pinecil arrived, build started same day.** Settings for this board: ~330°C for leaded solder, ~360°C for lead-free; tin the tip on first heat and keep it tinned.
...
2. **Volume sensing method**: load cell (accurate, recommended) vs pump-calibration-only (simpler, less accurate) vs inline flow sensor (pricey at low flow)? **New consideration (2026-09-07):** Rob is wary of 3D-printing the load-cell mounting bases (cantilever geometry with clearance gaps top/bottom is fiddly to get right — platform touching the base anywhere kills the reading). Two alternatives raised: **(a) gut a cheap 5kg digital kitchen scale per side** and solder the same raw load cell straight to the HX711, skipping the mounting-geometry problem entirely (Jarvis's pick — mechanically already solved, same cell, same amp). **(b) Count pump revolutions** (hall/optical encoder on the rotor) instead of weighing at all — no reservoir-side hardware, but drifts with tube wear/back-pressure so needs periodic recalibration. **Update 2026-09-10:** Rob went a third way — bought a pre-made AliExpress load-cell kit (metal bar + two perspex discs, no instructions) instead of DIY-printing or gutting a scale. Assembly confirmed: arrow sticker on the bar marks load direction (points down in use); one disc bolts under one end of the bar (base), the other bolts on top of the opposite end (platform) — a flat Z shape, bar floating free between the two discs with a gap (spacers/washers) so it can flex; a disc touching the bar anywhere but its own screwed end wrecks the reading. Wiring into the HX711: red→E+, black→E-, green→A+, white→A- (colours vary by maker). Still need to confirm cell capacity (1-2kg gives best resolution for a sub-1kg reservoir; 5kg still works, just coarser).
