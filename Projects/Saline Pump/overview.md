...
- 2026-09-11: **both pumps running on the assembled board** (the FET deaths were solder-joint intermittents, fixed by rework). Enclosure printing. Firmware Stage 6 written: gauge UI on the GC9A01s (port of `gauge-mockup.html`) + dosing model, in `firmware/gauges/gauges.ino`. Rob has flashed Stage 6 and given me standing OTA access, so future firmware updates go out from here, not his IDE. Next: flow calibration (ML_PER_MIN_AT_100 timed-jug test), then encoders (Stage 7), then HX711 load cells replace the estimated reservoir level. Also owed: a browser-based mockup UI matching the gauges with a speed slider (0=60% since pumps stall ~55%) — first attempt overran and got cut off, needs a smaller-scoped retry.
