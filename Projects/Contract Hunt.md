...
- **Angle worth using (14 Sep 2026):** "I build agentic AI systems in production" is a strong, differentiated line right now, backed by a working demo (this Jarvis setup). See [[Jarvis Productisation]] for the fuller idea (currently parked, not a project).
...
- 2026-09-06 (21:30): **instant application receipts added.** Rob felt he was missing updates (the ARC/IO Associates receipts only surfaced in evening briefs he'd skimmed), so `contract-hunt.sh` now pings a one-line Telegram receipt the moment each application is sent, on top of the evening-brief digest. Failed applies still ping separately. Also checked the weekend's only 6 (Opus Recruitment "Software Engineer" via LinkedIn alert, £53-60/hr): full ad reads **inside IR35**, correctly not applied; email-alert 6s stay digest-only since they can't be auto-applied anyway. Scoreboard unchanged: 10 sent, 0 replies (weekend, expected).
- 2026-09-07 (18:38): **first apply-failure ping.** An 8-scoring IO Associates Senior Node/TS repost (£500-540 remote) was pulled from JobServe between scrape and apply, so the form 404'd; turned out to be the same fintech role already applied to on 5 Sept at £530, so nothing lost. Bug found while diagnosing: on failure the script had been filing the last *successful* apply's screenshot as the result, which could have reported a send that never happened. Fixed: failures now screenshot the actual stuck page, stale screenshots cleared each run. Scoreboard still 10 sent, 0 replies.
- 2026-09-14 (13:10): **AI/agentic roles added to the hunt on Rob's instruction.** Four new JobServe searches (ai engineer, agentic ai, llm engineer, ai consultant; run now ~11 terms, ~33 min, fine on the 2h cron). Judge prompt updated: AI/agentic-systems roles are now equally a target, scored like core-stack roles; the honest pitch is Rob's year of running a production agentic system (LLM agents, memory, tool use, scheduled autonomy, integrations). Python-first roles cap at 7 when the substance is agent/LLM system design; model-training/data-science-first roles stay rejects. `auto-apply.sh` cover-note skill list extended to match so the claim is available (and true) in applications. Same day: Kernow Labs site gained an "AI & Agents" section as the public face of the same positioning.
