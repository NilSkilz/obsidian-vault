...
### A Commit Is Not Live Until the Process Restarts
**Learned:** 2026-08-28, the Aimee first-contact incident
...
**Corollary (2026-09-07):** same pattern, no restart needed to trigger it — the Tide videos retheme job hit the ~15-min turn wall and got cut off mid-report, even though the work (retheme, thumbnails, System tab link) had fully landed. Confirms the rule generalises to any long single-turn job, not just ones that restart a process. On the "should I use long-lived subagents for this" question Rob asked in response: subagents die with the parent run, so they only buy parallelism inside one turn, not a way past the wall-clock limit. For a job that's genuinely going to run long, the right shape is a detached process (nohup, writing to a log file) fired and left running, with the reply sent immediately and the log checked on the next message, rather than trying to hold the work inside one turn.

**Corollary 2 (2026-09-13), the detached process is not the whole answer:** first real use of the nohup shape was the saline pump OTA watcher, a script that swept the LAN every 45s waiting for the ESP32 to be powered on, pushed the firmware, then Telegrammed Rob. It did the hard part perfectly (found the board at 15:07, flashed it) and then got killed with the session mid-run, before the confirmation message. So the board was on the new firmware and Rob had no idea; he asked me to retry a job that was already done. **A detached process can do the work, but anything whose entire value is "and then tell Rob" has to live somewhere that outlives the session, i.e. cron.** Second habit that falls out of this: before re-running any job Rob asks you to retry, go and ask the *end state* whether it's already done (the device's own `/status`, git log, the live bundle). Cheap check, and here it was the whole answer.
