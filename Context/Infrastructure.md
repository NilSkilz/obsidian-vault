...
## Backups (guest VMs/CTs)

- **Primary (deep history):** one native Proxmox `vzdump` job (`backup-cf999020-84ea`), **all guests, Sundays 01:00**, mode snapshot, zstd, → storage **`data1-backups`** (`/mnt/data1/backups`, 440GB disk `sdb`). Retention **keep-daily=7, keep-weekly=4, keep-monthly=3** (+ keep-last=3). Emails `rob_stokes@me.com`. This is the authoritative archive and the recovery path (see the SAB-corruption lesson).
- **Secondary (latest-only mirror on `data2`):** `data2` is a separate 110GB disk (`sdc`, `/mnt/data2`), **NOT a Proxmox storage** (deliberately — Proxmox doesn't manage it). Mirrored by **`/usr/local/bin/mirror-backups-to-data2.sh`** via cron **`/etc/cron.d/mirror-data2`, Sundays 02:30** (after the vzdump window). Because data2 is small, the mirror keeps only the **latest backup of each guest** (one generation, ~25G), not data1's full retention: `rsync --files-from` the newest archive+`.log`+`.notes` per guest, then prune anything on data2 not in that keep-set. Logs to `/var/log/mirror-data2.log`; aborts if `/mnt/data2` isn't mounted. Set up 2026-07-11 (Rob thought a copy was already automated — it wasn't, the pre-existing data2 mirror was a manual one-off).
- **Offsite (crown jewels, encrypted+versioned):** the irreplaceable data is tiny (<100MB total: Vaultwarden 0.7MB, Paperless docs, MC DB 1.3MB, Kuma 6.9MB), so offsite is a `restic` job, not PBS. **`/usr/local/bin/offsite-backup.sh`** (cron `/etc/cron.d/offsite-backup`, **daily 03:00**) bundles: Vaultwarden `db.sqlite3`+`rsa_key.pem`(+attachments), MC `family.sqlite`, Kuma `kuma.db`, Paperless via `document_exporter` (docs+metadata, not raw Postgres). sqlite DBs are WAL-folded on the host (`sqlite3 wal_checkpoint(TRUNCATE); VACUUM`) for consistent, non-torn snapshots. restic encrypts client-side (AES-256) + dedups + versions; retention keep-daily=7/weekly=4/monthly=6. Config `/root/.config/offsite/offsite.env`, repo password `/root/.config/offsite/restic-pw` (chmod 600). Log `/var/log/offsite-backup.log`. **Restore verified 2026-07-11** (all sqlite pass `integrity_check`, key + Paperless manifest present).
  - **Home Assistant (VM101) IS included (2026-07-11).** HAOS is a VM (no `pct exec`), so it goes via HA's native backup: `/usr/local/bin/ha-backup.py` (WebSocket API, `python3-websockets`) generates a **config-only backup with `include_database:false`** (skips the ~62MB regeneratable recorder history), the script downloads it (`GET /api/backup/download/<id>?agent_id=hassio.local`, allowed with the core long-lived token), then deletes it from HA so they don't pile up on HAOS. Best-effort: an HA outage logs a WARN and doesn't block the other services. The tar carries `secrets.yaml`, `configuration.yaml`, and `.storage/*` (auth/users + all integration config) — verified restored from B2. **Gotchas:** the `/api/hassio/` supervisor proxy is **401 for long-lived tokens** (use the native `backup/*` WS API instead); the local backup agent id is **`hassio.local`**, not `backup.local`. HA token+URL added to `offsite.env` (`HA_TOKEN` long-lived, exp 2098).
  - **LIVE on Backblaze B2 (2026-07-11).** Two repos, same password: `LOCAL_REPO=/mnt/data1/backups/offsite-restic` (fast, same-host hot copy) and `OFFSITE_REPO=b2:tide-backup:crown-jewels` (Backblaze B2, native `b2:` backend, region us-west-001, bucket-scoped app key). Each run backs up to local then `restic copy` → B2, so snapshots are identical. **Restore verified end-to-end directly FROM B2** (all sqlite pass integrity_check, key + Paperless manifest present); compresses to ~1.6MB on the wire. B2 free tier is 10GB, so this is £0. Creds in `offsite.env` (`B2_ACCOUNT_ID`/`B2_ACCOUNT_KEY`, bucket-scoped, low blast radius).
  - **RESTIC ENCRYPTION KEY (the master key that decrypts every offsite backup):** `pUZMGYrCIibPFCayDTnaZ9wyrh4V8xhYiPxXgRCY7UAR`. Stored here in the vault as the **independent out-of-band custody copy** (Rob's decision 2026-07-11; he accepts the risk, GitHub repo is private + 2FA/passkey). It must live SOMEWHERE OTHER than Vaultwarden, because Vaultwarden is itself inside this backup, without the key, a restore of Vaultwarden is impossible. Rob is consolidating his phone password manager into Vaultwarden, so the vault is the reliable independent copy. **Do not let a Vaultwarden entry become the only home for this key.** Copy on the host at `/root/.config/offsite/restic-pw`.
  - **Retention gotcha (fixed, worth remembering):** `restic forget` groups by `host,paths` by default. The staging dir MUST be a stable path (`/var/tmp/offsite-stage`, not `mktemp`), else every run is a new "path" = its own series = never pruned. Belt-and-braces the forget also runs `--group-by host,tags`. See Technical Lessons.
- **Caveat:** the data1/data2 mirror is *same-host* — protects against disk failure / fat-fingered deletes, NOT the box dying, theft, or fire. The restic offsite job is the real DR path once pointed at S3.
- **Not covered by vzdump internals:** app data living in Docker named volumes (Vaultwarden, Paperless, Plausible, Kuma) is inside each CT's rootfs, so the CT-level vzdump does capture it — but app-level exports (e.g. Paperless `document_exporter`, Vaultwarden data dir) are the cleaner restore path for those.

## Home network map

> **Service-CT IP addressing (belt-and-braces):** **most CTs already have DHCP reservations on the Dream Machine** (Rob confirmed) — which is why they stayed on their low IPs. Seerr was the exception (no reservation → drifted on DHCP → seerr.cracky.co.uk 502'd, 2026-07-03). Fix: on top of the DM reservations, **all service CTs are now also pinned static in their `/etc/pve/lxc/<id>.conf`** (`net0 ... ip=192.168.1.X/24,gw=192.168.1.1`). Having both is fine and intended — a CT with a static config never asks DHCP, so the reservation is just a matching record; the Proxmox static always wins.
>
> **CRITICAL (learned the hard way 2026-07-11):** the **DHCP pool is `.6`–`.254`** (confirmed on the UDM), so it **overlaps every low static service IP**. A Proxmox static IP is NOT protected by itself — if there's no matching DM reservation, the UDM will hand that same IP to a phone/laptop → **IP conflict → ARP poisoning → intermittent 502s** through NPM (NPM's ARP for the IP flips to the other device's MAC while the CT still answers other hosts). This is exactly what bit **vaultwarden (.17)**: an iPhone got a dynamic lease on .17, NPM cached the phone's MAC, vault.cracky.co.uk 502'd. The newer CTs — **.16 MC, .17 vaultwarden, .18 paperless, .19 kuma — had NO reservation** (only .2–.14 did). **Fixed 2026-07-11: added DM reservations for all four** (mac→fixed_ip via the UniFi API, see below). Belt-and-braces means **BOTH** the Proxmox static AND a DM reservation; a static alone inside the pool is a time bomb.
>
> **DM reservation writes via API DO work now** (supersedes the old "301 wall" note): `PUT /proxy/network/api/s/default/rest/user/<_id>` with `{use_fixedip:true, fixed_ip, network_id}` (get `_id` from `stat/user/<mac>`, `network_id` = Default net `63ae2ed52b62cf07a84172fc`). Key `X-API-KEY` in `~/.config/jarvis/unifi.env`. **To evict an active conflicting lease** (a phone already squatting the IP): reservation alone won't bump an unexpired 24h lease — do `cmd/stamgr` `block-sta` then `unblock-sta` on the offending MAC to force a full re-DHCP; the reservation then pushes it onto a different IP. Then flush NPM's stale ARP: `pct exec 108 -- ip neigh flush <ip>` + a ping to re-resolve. `.conf` edits apply at next boot; running leases already matched so no restart was needed except Seerr (rebooted off its pool address). Backups at `*.conf.bak-ipfix`.
...
| Paperless-ngx | http://192.168.1.18:8000 | Document management (CT 114, static IP, installed 2026-07-10). **LAN-only, deliberately not exposed** (family paperwork). Login **rob** (pw in section below / Vaultwarden). |
| Uptime Kuma | http://192.168.1.19:3001 | Service monitoring (CT 115, static IP, installed 2026-07-10). **LAN-only.** Admin **Rob** (Jarvis-managed pw in `~/.config/jarvis/kuma.env`). 20 monitors + Telegram alerts live — see section below. |
...
- LXC at **192.168.1.18** (static in `/etc/pve/lxc/114.conf`), hostname `paperless`, 2 CPU / 4GB / 30GB on `data1-backups`, unprivileged with `nesting=1,keyctl=1`. Debian 13 base, Docker 29.6.1.
- Runs **Paperless-ngx** (`ghcr.io/paperless-ngx/paperless-ngx:latest`) via Compose at `/opt/paperless`. Five containers, all `restart: always`: webserver (`:8000`), Postgres 17, Redis 7, **Gotenberg 8 + Apache Tika** (so it OCRs/indexes Office docs, not just PDFs). OCR language `eng`, TZ `Europe/London`.
- **LAN-only by design** — no NPM proxy host, not on the public internet (family documents). Reachable at `http://192.168.1.18:8000`. If remote access is wanted later, do it via **Tailscale**, not another public subdomain. Adding a public NPM host would be one API call on cert 2 if that decision changes.
- Web login **rob / `siC5Seodf172Cyq0wG`** (set at install via `PAPERLESS_ADMIN_PASSWORD`; repo is private, consistent with the other service creds here). Change in-app under Admin; worth also saving into Vaultwarden. Token API works: `POST /api/token/`.
- **Backup-critical:** the named Docker volumes `paperless_data` and `paperless_media` (the document store + search index) plus the Postgres volume `paperless_pgdata`. Config/secrets in `/opt/paperless/paperless.env` + `.env` (chmod 600). `PAPERLESS_SECRET_KEY` and DB password live there. Covered by the weekly `vzdump` of CT 114; a doc-level export is also available via `document_exporter`. **This is exactly the kind of data the PBS/offsite-backup proposal is about** (see `Ops/Proposals.md`).
- Ingest: drop files into the `consume/` bind mount (`/opt/paperless/consume`, polled every 60s, recursive) or upload via the web UI / mobile app.
- Upgrades: `cd /opt/paperless && docker compose pull && docker compose up -d`. NOT covered by `updates.sh` (Docker images, same caveat as Plausible/Vaultwarden).

## Uptime Kuma (CT 115, installed 2026-07-10)

- LXC at **192.168.1.19** (static in `/etc/pve/lxc/115.conf`), hostname `uptimekuma`, 1 CPU / 1GB / 8GB on `data1-backups`, unprivileged with `nesting=1,keyctl=1`. Debian 13, Docker 29.6.1.
- Single container `louislam/uptime-kuma:1` via Compose at `/opt/uptime-kuma`, port `3001:3001`, data in named volume `uptime-kuma_uptimekuma` (`/app/data`, SQLite — **backup-critical**, holds monitors + history + settings). `restart: always`.
- **LAN-only**, reachable at `http://192.168.1.19:3001`.
- **Admin:** Rob claimed it in the browser (username `Rob`), then asked me to manage it, so I reset the password. **Creds in `~/.config/jarvis/kuma.env`** (`KUMA_USER=Rob`, `KUMA_PASS=...`). Password resets are done by hashing in-container (`docker exec ... node -e '...bcryptjs.hashSync(argv[1],10)...'` → write to a file → `sqlite3 ... UPDATE user SET password=trim(readfile('/app/data/_h'))`). **Do NOT pass a bcrypt hash on a shell command line** — the `$2a$...` gets mangled by shell `$`-expansion through ssh; always route the hash through a file.
- **20 monitors configured + Telegram alerts live** (2026-07-10). Set up via the `uptime-kuma-api` Python client — venv at `~/.local/share/kuma-venv`, reusable script at `~/Jarvis/kuma/kuma_config.py` (idempotent: skips monitors that already exist by name). Monitors: 16 TCP/port checks (Proxmox, UniFi, Plex, HA, Prowlarr, SAB, Sonarr, Radarr, Jarvis chat bridge, Seerr, Tdarr, NPM, Plausible, MC, Vaultwarden, Paperless) + 4 HTTPS public-path checks (vault/tide/plex/plausible.cracky.co.uk, to catch cert/DNS/proxy issues end to end). All 60s interval, 2 retries. First run: all 20 UP.
- **Telegram notifications** via Kuma's native support, wired to every monitor + set default. Bot `@haven_ai_chatbot`, token + chat id `7331133695` in **`~/.config/jarvis/telegram.env`** (rebuilt here from the token Rob re-supplied 2026-07-10; the old `telegram-bridge.env` didn't survive the migration). Test alert delivered OK. **This token can seed the Telegram bridge rebuild** (see the retired-tooling section).
- **Supersedes the hand-rolled heartbeat alerting** for up/down of services (heartbeat scripts still do thin-pool %, updates, task checks — Kuma only does service reachability).
- Upgrades: `cd /opt/uptime-kuma && docker compose pull && docker compose up -d`. Not covered by `updates.sh` (Docker image).
- **On the Tide System page** (parents only, LAN link).
...
- **Telegram bridge** — custom `telegram-notify.sh` / `telegram-poll.sh` scripts (NOT the MCP plugin). Bot `@haven_ai_chatbot`, chat id `7331133695`. **Token re-supplied by Rob 2026-07-10 and now stored in `~/.config/jarvis/telegram.env`** (for Uptime Kuma alerts) — so the bridge rebuild no longer needs a new token, just the scripts. **Durable lesson:** Telegram allows only ONE `getUpdates` consumer, so the `telegram@claude-plugins-official` plugin and a custom poller fight over the bot token (409 Conflict) and inbound silently dies. Keep the plugin disabled if the custom bridge is rebuilt.
