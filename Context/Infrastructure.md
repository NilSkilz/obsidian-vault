...
- ~~**HA control**~~ — **rebuilt 2026-07-03**: `Jarvis/bin/ha-api.sh` (states / state / call / raw), long-lived token in `~/.config/jarvis/ha.env`, 43 entities visible. Token does NOT reach the Supervisor proxy (`/api/hassio/*` 401s), so add-on management and configuration.yaml edits still need Rob or an SSH add-on.
