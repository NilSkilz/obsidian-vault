...
| Uptime Kuma | http://192.168.1.19:3001 | Service monitoring (CT 115, static IP, installed 2026-07-10). **LAN-only.** Admin account **claimed on first visit** (Plausible-style) — see section below. |
...
- **First-run setup outstanding:** Uptime Kuma claims its admin account on first browser visit (like Plausible's `/register`). **Rob (or I) needs to hit `http://192.168.1.19:3001` once and set the admin user/pass** before it's useful. Until then it 302-redirects to `/setup`.
- **Monitors + alerts not configured yet** — that's the next step: add HTTP/TCP monitors for every service (the *arrs, Plex, HA, NPM, Plausible, Vaultwarden, Paperless, MC, Proxmox, HAOS) and wire a **Telegram notification** (Kuma has native Telegram support: bot token + chat id `7331133695`, bot `@haven_ai_chatbot` — token needed, the old `telegram-bridge.env` didn't survive). Once live, Kuma can take over the alerting the hand-rolled heartbeat scripts do now.
