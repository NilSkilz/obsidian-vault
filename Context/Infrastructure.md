...
- **Offsite (crown jewels, encrypted+versioned):** the irreplaceable data is tiny (<100MB total: Vaultwarden 0.7MB, Paperless docs, MC DB 1.3MB, Kuma 6.9MB), so offsite is a `restic` job, not PBS. **`/usr/local/bin/offsite-backup.sh`** (cron `/etc/cron.d/offsite-backup`, **daily 03:00**) bundles: Vaultwarden `db.sqlite3`+`rsa_key.pem`(+attachments), MC `family.sqlite`, Kuma `kuma.db`, Paperless via `document_exporter` (docs+metadata, not raw Postgres). sqlite DBs are WAL-folded on the host (`sqlite3 wal_checkpoint(TRUNCATE); VACUUM`) for consistent, non-torn snapshots. restic encrypts client-side (AES-256) + dedups + versions; retention keep-daily=7/weekly=4/monthly=6. Config `/root/.config/offsite/offsite.env`, repo password `/root/.config/offsite/restic-pw` (chmod 600). Log `/var/log/offsite-backup.log`. **Restore verified 2026-07-11** (all sqlite pass `integrity_check`, key + Paperless manifest present). **HA (VM101) config/secrets not yet included** — needs the HA backup API, follow-up.
